...
public class TestBoardDAOImpl {
	...
	@Test
	public void testReadArticleById() {
		int testTargetId = 11;
		ArticleVO articleVO = new ArticleVO();
		//글을 읽음
		articleVO = boardDAO.readArticleById(testTargetId);
		Assert.notNull(articleVO);
		//글을 읽고 나서 삭제.
		//boardDAO.deleteArticleById(testTargetId);
	}
	
	@Test
	public void testGetAllArticleList() {
		ArticleVO articleVO = new ArticleVO();
		
		List<ArticleVO> list = boardDAO.getAllArticleList();
