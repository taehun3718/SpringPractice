		...
		$("#usrId").keyup(function() {
			$.post("<c:url value="/member/checkDuplicateUserIDAjax"/>"
				...
				if(data=="true") {
					...
					$("#duplicateResult").text("사용 가능한 ID입니다.");
				}
			});
		});
...
<form:form commandName="memberInfoVO" id="joinForm" name="joinForm">
	ID : <input type="text" name="usrId" id="usrId" maxlength="20" value="${memberInfoVO.usrId }"/>
		...
		</span><form:errors path="usrId"/><br/>
	Email : <input type="text" name="emailId" id="emailId" value="${memberInfoVO.emailId }" /> @ <input type="text" name="emailHost" id="emailHost" value="${memberInfoVO.emailHost }"/>
		<form:errors path="emailId"/>
		<form:errors path="emailHost"/><c:out value="${emailError }"/><br/>
